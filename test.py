...
def train_one(save_path, config, log_file_dir, index, logfile_level, console_level):
    ...
    return Trainer(config, save_path=save_path).train()


train_one(save_path="./model", config=config, log_file_dir="./tensorboard", index="0", logfile_level=logging.DEBUG, console_level=logging.INFO)
